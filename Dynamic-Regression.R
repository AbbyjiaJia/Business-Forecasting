...
vic_elec_daily <- vic_elec %>%   
  filter(year(Time) == 2014) %>% # filter 2014
...
# so monotonic transformations will not work - need to do something else
# In fact the models used are much more complicated 
# cubic splines (similar to picewise linear) are used instead of quadratic terms
# a separate model for each day of the week - days are very different 
# separate models for seasons

fit %>% report() # Count the number of parameters estimated
fit %>% tidy() # tidy() can help

augment(fit) %>%
  features(.resid, ljung_box, dof = 9, lag = 21)
# Again 2 or 3 seasonal periods
# Plenty of data here so we do 3*7


# If this was a consulting job I would spend a bit 
# more time to improve the model
# DO NOT RUN NOW

fit_better <- vic_elec_daily %>%
  model(ARIMA(Demand ~ Temperature + I(Temperature^2) + # I() treats this a new variable rather than interaction
                (Day_Type=="Weekday")+ 
                pdq(0:7), order_constraint = p + q + P + Q <= 10,
              stepwise = FALSE))

fit_better %>% report()
fit_better %>% gg_tsresiduals()
  
# Let's forecast one-day ahead 
vic_next_day <- new_data(vic_elec_daily, 1) %>%
  mutate(Temperature = 26, Day_Type = "Holiday") # New Year's day
...
vic_elec_future <- new_data(vic_elec_daily, 28) %>%
  mutate(
    Temperature = c(rep(36,14),rep(25,14)), #
...
# Not surprising that we need all terms to deal with this complicated
...
# each of 26 models
fit_gas <- us_gasoline %>%
  model(!!!model_defs) # Bang bang bang operator grabs the model definitions
...
calls %>% fill_gaps() %>%
  gg_season(volume, period = "day", alpha = 0.6) #+ # make transparent
#  guides(colour = FALSE)
